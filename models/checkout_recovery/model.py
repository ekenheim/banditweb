"""
checkout_recovery.py — Checkout Recovery contextual bandit (self-contained)
────────────────────────────────────────────────────────────────────────────
LinUCB variant for learning which checkout intervention works best
given cart value as context. 1-dim context: normalized cart value [0, 1].
...
class CheckoutRecoveryBanditModel(BanditBase):

    MLFLOW_EXPERIMENT = "bandit-checkout-recovery"

    def _init_state(self, params: dict) -> None:
        self.alpha = float(params.get("alpha", 0.8))
