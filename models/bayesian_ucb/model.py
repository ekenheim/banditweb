"""
bayesian_ucb.py — Bayesian UCB bandit policy (self-contained)
─────────────────────────────────────────────────────────────
UCB with Beta posterior credible intervals:
    score(a) = mean(a) + c * std(a)
where mean and std come from the Beta(alpha, beta) posterior.
"""
...
class BayesianUCBModel(BanditBase):
    ...
    def _select_arm(self, context=None) -> int:
        ...
        return int(np.argmax(mean + self.c * std))
    ...
    def state_dict(self) -> dict:
        ...
        std = np.sqrt(self.alpha * self.beta / (ab * ab * (ab + 1)))
        ucb = mean + self.c * std
        return {f"value_{i}": float(mean[i]) for i in range(self.n_arms)} | \
