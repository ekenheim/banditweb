"""
pricing.py — Dynamic Pricing bandit policy (self-contained)
────────────────────────────────────────────────────────────
Thompson Sampling for learning optimal price point.
Reward is revenue = price * purchase (non-binary).

Uses Beta posteriors to model purchase probability per price point,
then selects the arm maximizing expected revenue = price * E[p(purchase)].
"""
...
class PricingBanditModel(BanditBase):

    MLFLOW_EXPERIMENT = "bandit-pricing"

    def _init_state(self, params: dict) -> None:
        self.alpha = np.ones(self.n_arms, dtype=np.float64)
        self.beta = np.ones(self.n_arms, dtype=np.float64)
        # Price points for each arm (read from params or use defaults)
        prices_str = params.get("prices", "19.99,29.99,39.99,49.99,59.99")
        self.prices = np.array([float(p) for p in prices_str.split(",")])

    def _select_arm(self, context=None) -> int:
        # Sample purchase probability from Beta posterior, then pick arm maximizing expected revenue
        samples = np.random.beta(self.alpha, self.beta)
        expected_revenue = samples * self.prices[:self.n_arms]
        return int(np.argmax(expected_revenue))

    def _update(self, arm: int, reward: float) -> None:
        # Reward is revenue = price * purchase. Recover purchase signal.
        purchased = 1 if reward > 0 else 0
        self.alpha[arm] += purchased
        self.beta[arm] += 1 - purchased

    def state_dict(self) -> dict:
        posterior_mean = self.alpha / (self.alpha + self.beta)
        expected_rev = posterior_mean * self.prices[:self.n_arms]
        return {f"value_{i}": float(expected_rev[i]) for i in range(self.n_arms)} | \
               {f"purchase_rate_{i}": float(posterior_mean[i]) for i in range(self.n_arms)} | \
